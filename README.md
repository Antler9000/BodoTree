# BodoSearchTree
"C와 C++로 구현하는 자료구조" 책에서 소개된 이진 탐색 계열 트리 자료구조들을 C++로 구현한 리포지토리입니다 [1].  
...
## 3. 사용 방법
### 3.1. 프로젝트 사용 방법
본 트리 자료구조들은 라이브러리가 아닌 오직 헤더 파일로만 구현되어있습니다.

1. 사용하고자 하는 트리의 헤더 파일을 프로젝트 디렉토리 혹은 추가 포함 디렉토리에 둡니다.  
(ex : `SplayTree.h`)

2. `Common` 폴더 내 모든 헤더 파일들도 마찬가지로 프로젝트 디렉토리 혹은 추가 포함 디렉토리에 둡니다.  
(ex : `BSTTemplate.h`, `Stack.h`, `Debug.h`)

3. 코드 내에서는 사용하고자 하는 트리의 헤더 파일을 전처리로 포함하고서 트리를 선언하여 사용합니다.  
    ```cpp
        //ex code
        #include "SplayTree.h"
...
### 3.2. 테스트 실행 방법
테스트는 Visual Studio 2022, 2026을 기준으로 구현되었습니다.
...
- **퍼블릭 메소드 예외 안전성 표기**
    - noexcept가 아닌 각 퍼블릭 메소드들의 예외 안전성 수준을 알 수 있도록 표기하기 (강함, 보통)
...
- **iterator 제공**
    - 트리들을 지원하는 하나의 iterator 클래스를 제공하기

<br>

- **emplace 메소드**  
   - 가변 인자 템플릿을 이용하여 트리 자료구조 내에서 바로 객체를 생성할 수 있도록 하기

<br>

- **메모리 풀 제공**  
    - 메모리 풀로부터 연속된 공간 속에서 배정받을 수 있도록 하여 동적 할당 횟수를 줄이고 캐시 히트율을 높이기
...
- **이동 연산 최적화 분기**  
    - 스택의 내부 어레이가 확장될 때 이동 연산의 예외 안전성 여부를 확인하여 가능하면 복사 대신 이동을 사용하도록 최적화하기

<br>

- **노드의 부모 포인터**  
    스택을 이용하지 않고 순회 및 조정을 할 수 있도록 하여 예외 안전성 높이기

<br>

- **키 타입 템플릿**
    템플릿 타입 인자가 비교자를 가졌는지를 확인할 수 있도록 하여, 임의의 키 타입을 받을 수 있도록 템플릿화하기
...
## 6. 구현 제외
- **이진 힙, B+ 트리, 트라이 자료구조**  
    이진 탐색 계열의 트리에 해당하지 않기에 제외하였습니다.
