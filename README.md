...
4. [성능 측정 환경](#4-성능-측정-환경)

5. [성능 측정 방식](#5-성능-측정-방식)
...
### 7.0. 공통
- 포워딩을 적용한 메소드들에서, 단순한 데이터 타입에 대해서는 참조 전달이 아니라 값복사를 사용하도록 하여 참조 비용을 줄이기
...
### 7.1. 재귀 호출을 이용한 이진 탐색 트리
- 없음

<br>

### 7.2. 반복문을 이용한 이진 탐색 트리
- 없음
...
### 7.6. 힙
- 최소힙과 최대힙을 폴리몰피즘으로 구분하지 말고 더 정적인 방식으로 구현하기
...
- ### 트리의 높이보다 중요한 것은 지역성이다

### 8.1. 재귀 호출을 이용한 이진 탐색 트리

- ### 퍼블릭 메소드와 재귀 구현 메소드를 구분하자

- ### 꼬리 재귀 최적화가 일어나도록 하자

### 8.2. 반복문을 이용한 이진 탐색 트리

- ### 친구의 자식은 내 친구가 아니다

- ### 트리를 상속하고 싶다면 노드 타입을 분리하자

- ### 노드의 콘솔 출력은 << 오퍼레이터 오버로딩으로 수행하자

- ### 제너릭 메소드는 어렵다

- ### 후위 순회는 전위 순회보다 어렵다

- ### 회전을 이용한 소멸자는 예외에 안전하다
