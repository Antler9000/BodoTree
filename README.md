...
부모노드의 키(key)는 좌측 자식 노드의 키보다는 크고, 우측 자식의 키보다는 작아야 한다는 조건을 통해 빠르게(평균적으로 O(log(n))) 특정 키의 노드를 찾을 수 있습니다.  
...
탐색의 타겟이 된 노드를 루트로 올려, 자주 탐색되는 노드와 그 인접 노드들이 더 빨리 탐색되도록 합니다.  

2단원에서 사용한 이진 탐색 트리를 상속 받고, 이러한 조정 과정을 스택을 이용하여 추가 구현하였습니다.
...
노드의 왼쪽 하위 트리의 높이와 우측 하위 트리의 높이의 차가 2 이상이 될 경우에, 해당 위치를 기준으로 노드들을 회전하여 트리의 균형을 조절합니다.  

앞선 스플레이 트리와 마찬가지로, 2단원의 이진 탐색 트리를 상속 받고 스택을 이용하여 추가적인 조정 과정을 구현하였습니다.
...
레드-블랙 트리는 이진 탐색 트리의 노드들에 색깔을 부여하여 여러 데이터가 들어간 노드들을 모방합니다.

레드-블랙 트리는 2-3-4 트리와 달리 균형을 완벽하게 유지하지는 못하지만, 이진 탐색 트리의 로직을 재활용해 쉽게 구현이 가능합니다.

앞선 트리들과 마찬가지로, 2단원의 이진탐색트리를 상속받고 추가적인 로직들을 스택을 이용하여 구현하였습니다.
...
6. 예제를 실행하고자 한다면, Visual Studio 상단의 구성(configuration)을 "Debug" "x64"로 설정하고, 디버깅 시작키인 'F5'를 누르면 됩니다.  
실행 결과가 검은 명렁 프롬프트에 출력될 것입니다. 또한 메모리 누수가 존재한다면 Visual Studio 하단의 출력(output) 창에 누수 정보가 출력될 것입니다.

7. 테스트를 실행하고자 한다면, 상단의 구성을 "Release"로 변경하고, 디버깅 없이 실행('Ctrl+F5')을 누르면 됩니다.  
테스트 과정과 결과가 검은 명령 프롬프트에 출력될 것입니다.
