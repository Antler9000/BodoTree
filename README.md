...
## 5. 구현 예정
- **제너릭 메소드 인라이닝 강화**
    - 함수 포인터가 아니라 함수 객체 템플릿을 이용하여 하위 작업 메소드 호출이 더 잘 인라이닝될 수 있도록 하기.
...
## 6. 구현 제외
- **emplace 메소드**  
    emplace 메소드는 트리 자료구조 내에서 바로 객체를 생성할 수 있어 유용하지만, 가변 인자 템플릿 방식이 필요하고 각 단원에 구현이 필요하므로 현재 범위에서 제외하였습니다.

<br>

- **노드의 부모 포인터**  
    노드에 부모 포인터가 있다면 스택을 이용하지 않고 순회 할 수 있어 예외 안전성이 높아집니다. 그러나 이를 수정하기 위해선 전 단원에 걸쳐 작업을 진행해야 하기에 제외하였습니다.

<br>

- **메모리 풀**  
    메모리 풀을 사용한다면 삽입마다 발생하는 동적 할당 비용을 줄일 수 있지만, 가비지 콜렉션이 추가로 필요하므로 제외하였습니다.

<br>

- **세부적인 타입 구분 최적화**  
    단순한 데이터 타입에 대한 값전달 방식 사용, 데이터 타입의 이동 연산이 예외에 안전한지에 따른 최적화 분기 등은 자료구조 입문이라는 본 프로젝트의 취지에서 벗어나 심화된 C++ 기능 학습과 사용이 주가 되므로 제외하였습니다.
    
<br>

- **B+ 트리, 트라이 자료구조**  
    이진 탐색 트리와 다른 별도의 테스팅을 요구하는 트리들이라 구현과 유지 보수가 힘드리라 생각해 제외하였습니다.
