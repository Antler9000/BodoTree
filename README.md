...
### 1.4. 스플레이 트리
스플레이 트리(Splay Tree)는 특정 노드를 검색할 때마다 해당 노드를 루트에 가까운 위치로 올려, 자주 검색되는 노드가 더 빨리 검색되도록 하는 이진탐색트리의 일종입니다.
...
하나의 3-노드나 4-노드가 부모 자식의 관계인 여러 노드들로 구성되므로, 레드-블랙 트리는 2-3-4 트리와 달리 균형을 완벽하게 유지하지는 못합니다. 그러나 레드-블랙 트리는 2-3-4 트리의 삽입-삭제 로직을 구현하여 균형을 어느정도 유지하며, 이진 탐색 트리의 로직을 재활용해 비교적 쉽게 구현이 가능하다는 장점을 가집니다.
